"""
Script pour annoter les phrases générées avec origine, destination et positions.
"""
...
def find_city_in_text(text: str, city: str) -> List[Tuple[int, int]]:
    ...
    while True:
        ...
        # On doit trouver la correspondance dans le texte original
        # en tenant compte des différences de casse/tirets
...
def main():
    """Fonction principale."""
    logger.info("Début de l'annotation du dataset")
    ...
    input_file = Path("data/dataset/draft/sentences_raw.txt")
    if not input_file.exists():
        logger.error(f"Fichier {input_file} introuvable. Exécutez d'abord 02_generate_dataset_draft.py")
        return
    ...
    output_file = Path("data/dataset/draft/annotations.json")
    ...
    logger.info(f"\nStatistiques :")
    logger.info(f"  - Phrases valides : {valid_count}")
    logger.info(f"  - Phrases invalides : {invalid_count}")
    ...
    for i, annotation in enumerate(annotations[:3], 1):
        logger.info(f"  {i}. ID: {annotation['id']}, Valid: {annotation['is_valid']}, Origin: {annotation['origin']}, Dest: {annotation['destination']}")
