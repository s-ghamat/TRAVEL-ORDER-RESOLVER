"""
Script pour exporter le dataset annoté dans les formats JSONL (BERT) et CSV (validation).
"""
...
def load_annotations() -> List[Dict]:
    ...
    annotations_file = Path("data/dataset/draft/annotations.json")
...
def main():
    ...
    output_dir = Path("data/dataset/draft")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Exporter en JSONL (format BERT)
    logger.info("Export JSONL...")
    ...
    logger.info(f"  - Valides : {train_valid}")
    logger.info(f"  - Invalides : {train_invalid}")
    logger.info(f"Validation :")
    logger.info(f"  - Total : {len(validation_annotations)}")
    logger.info(f"  - Valides : {val_valid}")
    logger.info(f"  - Invalides : {val_invalid}")
    
    logger.info(f"\nExport terminé avec succès !")
    logger.info(f"Fichiers créés dans {output_dir} :")
    logger.info(f"  - train.jsonl")
    logger.info(f"  - validation.jsonl")
