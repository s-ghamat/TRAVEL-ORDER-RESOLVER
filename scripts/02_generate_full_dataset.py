"""
Script pour générer le dataset complet de 10 000 phrases à partir des templates.
"""
...
def main():
    """Fonction principale."""
    logger.info("=" * 60)
    logger.info("GÉNÉRATION DU DATASET COMPLET (10 000 phrases)")
    logger.info("=" * 60)
    ...
    valid_count = int(TOTAL_PHRASES * VALID_RATIO)
    invalid_count = int(TOTAL_PHRASES * INVALID_RATIO)
    ambiguous_count = TOTAL_PHRASES - valid_count - invalid_count
    
    logger.info(f"Répartition : {valid_count} valides, {invalid_count} invalides, {ambiguous_count} ambiguïtés")
    ...
    logger.info(f"\nGénération des phrases valides ({valid_count})...")
    valid_sentences = generate_valid_sentences(templates, cities, valid_count)
    
    logger.info(f"Génération des phrases invalides ({invalid_count})...")
    invalid_sentences = generate_invalid_sentences(templates, cities, invalid_count)
    
    logger.info(f"Génération des phrases avec ambiguïtés ({ambiguous_count})...")
    ambiguous_sentences = generate_ambiguous_sentences(templates, cities, ambiguous_cities, ambiguous_count)
    ...
    output_file = output_dir / "sentences_raw.txt"
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Total phrases : {len(all_sentences)}")
